...
OpenLayers.Element.removeClass(this.div,"oleFadeMap")},ignoreEvent:function(a){OpenLayers.Event.stop(a,!0)},getButton:function(a){var b=document.createElement("input");b.value=a;b.type="button";OpenLayers.Element.addClass(b,this.buttonClass);return b},CLASS_NAME:"OpenLayers.Editor.Control.Dialog"});/*
    https://github.com/geops/ole/blob/master/license.txt
 @link       https://github.com/geops/ole
*/
...
b,c);if(!a._moving){var f=a.dragControl.handlers.drag.evt,i=!(!a._setfeature&&a.preserveAspectRatio)&&!(f&&f.shiftKey),h=new OpenLayers.Geometry.Point(d,e),f=a.center;this.rotate(-a.rotation,f);h.rotate(-a.rotation,f);var k=this.x-f.x,g=this.y-f.y,j=k-(this.x-h.x),l=g-(this.y-h.y);a.irregular&&!a._setfeature&&(k-=(this.x-h.x)/2,g-=(this.y-h.y)/2);this.x=d;this.y=e;h=1;a.feature.geometry instanceof OpenLayers.Geometry.Point?g=1:i?(g=1E-5>Math.abs(l)?1:g/l,h=(1E-5>Math.abs(j)?1:k/j)/g):(j=Math.sqrt(j*
j+l*l),g=Math.sqrt(k*k+g*g)/j);a._moving=!0;a.box.geometry.rotate(-a.rotation,f);delete a._moving;a.box.geometry.resize(g,f,h);a.box.geometry.rotate(a.rotation,f);a.transformFeature({scale:g,ratio:h});a.irregular&&!a._setfeature&&(k=f.clone(),k.x+=1E-5>Math.abs(d-f.x)?0:this.x-d,k.y+=1E-5>Math.abs(e-f.y)?0:this.y-e,a.box.geometry.move(this.x-d,this.y-e),a.transformFeature({center:k}))}},f=function(b,c){var d=this.x,e=this.y;OpenLayers.Geometry.Point.prototype.move.call(this,b,c);if(!a._moving){var f=
a.dragControl.handlers.drag.evt,f=f&&f.shiftKey?45:1,h=a.center,g=this.x-h.x,i=this.y-h.y;this.x=d;this.y=e;d=Math.atan2(i-c,g-b);d=Math.atan2(i,g)-d;d*=180/Math.PI;a._angle=(a._angle+d)%360;d=a.rotation%f;if(Math.abs(a._angle)>=f||0!==d)d=Math.round(a._angle/f)*f-d,a._angle=0,a.box.geometry.rotate(d,h),a.transformFeature({rotation:d})}},i=Array(8),h=Array(4),g,l,m,n="sw s se e ne n nw w".split(" "),j=0;8>j;++j)g=this.box.geometry.components[j],l=new OpenLayers.Feature.Vector(g.clone(),{role:n[j]+
"-resize"},"string"==typeof this.renderIntent?null:this.renderIntent),0==j%2&&(m=new OpenLayers.Feature.Vector(g.clone(),{role:n[j]+"-rotate"},"string"==typeof this.rotationHandleSymbolizer?null:this.rotationHandleSymbolizer),m.geometry.move=f,g._rotationHandle=m,h[j/2]=m),g.move=b,g.resize=c,g.rotate=d,l.geometry.move=e,g._handle=l,i[j]=l;this.rotationHandles=h;this.handles=i}});OpenLayers.Editor.Control.FixedAngleDrawing=OpenLayers.Class(OpenLayers.Control,{CLASS_NAME:"OpenLayers.Editor.Control.FixedAngleDrawing",active:!1,sketchVerticesAmount:null,guides:null,initialize:function(a){this.guides=[];OpenLayers.Control.prototype.initialize.call(this);this.layer=a},activate:function(){var a=OpenLayers.Control.prototype.activate.call(this);if(a)this.layer.events.on({sketchstarted:this.onSketchStarted,sketchmodified:this.onSketchModified,sketchcomplete:this.onSketchComplete,scope:this});
...
oleLayerSettingsLegendHeader:"Legend",oleLayerSettingsOpacityHeader:"Opacity (%)",oleMergeFeature:"Merge selected geometry",oleMergeFeatureSelectFeature:"Please select at least 2 geometries.",oleModifyFeature:"Modify geometry",oleNavigation:"Navigation",olePixelUnit:"px",oleSelectFeature:"Select geometry",oleSnappingSettings:"Snapping settings",oleSnappingSettingsLayer:"Snapping layer",oleSnappingSettingsTolerance:"Snapping tolerance",oleSplitFeature:"Split selected geometry",oleTransformFeature:"Scale, rotate and move geometry",
oleCADTools:"CAD Tools",oleCADToolsDialogParallelDrawing:"Parallel Drawing",oleCADToolsDialogGuidedDrawing:"Guided Drawing",oleCADToolsDialogShowLayer:"Show Guide Lines",oleCADToolsDialogTolerance:"px tolerance",oleDialogCancelButton:"Cancel",oleDialogSaveButton:"Save",oleDialogOkButton:"Okay"});
